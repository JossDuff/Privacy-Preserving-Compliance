  ...
  try {
    ...
    const dualMembershipFormatter: InputFormatter = async (ctx) => {
      ...
      const [leavesA, leavesB] = await Promise.all([
        ...
        ctx.proofManager.fetchLeaves(ctx.definition.leavesHash2),
      ]);

      setStatus("Computing merkle proof...");
      const proofA = computeMerkleProofForLeaf(leavesA, BigInt(userAddr));
      const proofB = computeMerkleProofForLeaf(leavesB, BigInt(userAddr));
